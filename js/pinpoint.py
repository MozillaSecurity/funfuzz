...
def pinpoint(itest, logPrefix, jsEngine, engineFlags, infilename, bisectRepo, alsoRunChar=True, alsoReduceEntireFile=False):
    ...
    if bisectRepo is not "none":
        ...
        if not isBuildSlave and sys.version_info >= (2, 6) and testJsShellOrXpcshell(jsEngine) != "xpcshell" and os.path.join('build', 'dist', 'js') not in jsEngine:
            autobisectCmd = [sys.executable, autobisectpy] + valgrindX + ["-d", bisectRepo, "-i", "-p", "-a", archOfBinary(jsEngine), "-c", testDbgOrOpt(jsEngine), "--flags=" + ','.join(engineFlags) if engineFlags] + [infilename] + itest
