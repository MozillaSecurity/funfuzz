...
def random_flag_set(shell_path):  # pylint: disable=too-complex,too-many-branches,too-many-statements
    ...
    if shell_supports_flag(shell_path, "--spectre-mitigations=on") and chance(.3):
    ...
    if shell_supports_flag(shell_path, "--cpu_count=1"):
        if chance(.7):
            # Focus on the reproducible cases
            args.append("--ion-offthread-compile=off")
        elif chance(.5) and multiprocessing.cpu_count() > 1 and shell_supports_flag(shell_path, "--cpu-count=1"):
