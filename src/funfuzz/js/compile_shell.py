...
LOG_COMPILE_SHELL = get_logger(__name__)
LOG_CSHELL_NO_TERMINATOR = get_logger("cshell_no_terminator", terminator="")
...
class CompiledShell:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def main(cls, args=None):
        ...
        try:
            ...
            LOG_COMPILE_SHELL.error(repr(ex))
            return 1
    ...
    def run(argv=None):
        ...
        with LockDir(sm_compile_helpers.get_lock_dir_path(Path.home(), options.build_opts.repo_dir)):
            ...
            LOG_COMPILE_SHELL.info(shell.get_shell_cache_js_bin_path())
...
def cfgJsCompile(shell):  # pylint: disable=invalid-name,missing-param-doc,missing-type-doc
    ...
    LOG_COMPILE_SHELL.info("Compiling...")  # Output *with* a trailing newline to avoid breaking other stuff
...
def cfgBin(shell):  # pylint: disable=invalid-name,missing-param-doc,missing-raises-doc,missing-type-doc
    ...
    LOG_COMPILE_SHELL.debug("Command to be run is: %s %s",
                            " ".join(quote(str(x)) for x in env_vars),
...
def makeTestRev(options):  # pylint: disable=invalid-name,missing-docstring,missing-return-doc,missing-return-type-doc
    def testRev(rev):  # pylint: disable=invalid-name,missing-return-doc,missing-return-type-doc
        ...
        LOG_CSHELL_NO_TERMINATOR.info("Rev %s:", rev)
        ...
        LOG_COMPILE_SHELL.info("Testing...")
...
def obtainShell(shell, updateToRev=None, updateLatestTxt=False):  # pylint: disable=invalid-name,missing-param-doc
    ...
    if shell.get_shell_cache_js_bin_path().is_file():  # pylint: disable=no-else-return
        ...
        LOG_COMPILE_SHELL.info("Found cached shell...")
        ...
        if os.getenv("RETAIN_SRC"):
            LOG_COMPILE_SHELL.info("RETAIN_SRC is set to True, so recompiling with sources retained...")
    ...
    if use_s3cache and not os.getenv("RETAIN_SRC"):
        ...
        if s3cache_obj.downloadFile(f"{shell.get_shell_name_without_ext()}.tar.bz2",
            ...
            LOG_COMPILE_SHELL.info("Extracting shell...")
    ...
    try:
        if updateToRev:
            ...
            LOG_COMPILE_SHELL.info("Updating to rev %s in the %s repository...", updateToRev, shell.build_opts.repo_dir)
        ...
        LOG_COMPILE_SHELL.info("Compilation failed (%s) (details in %s)", ex, cached_no_shell)
