...
LOG_S3CACHE = get_logger(__name__)
...
class S3Cache:  # pylint: disable=missing-docstring
    ...
    def connect(self):  # pylint: disable=missing-return-doc,missing-return-type-doc
        ...
        try:
            ...
            LOG_S3CACHE.warning('Unable to connect via boto using profile name "%s" in ~/.boto', EC2_PROFILE)
            return False
        except boto.exception.S3ResponseError:
            LOG_S3CACHE.warning('Unable to connect to the following bucket "%s", please check your credentials.',
                                self.bucket_name)
            return False
    ...
    def downloadFile(self, origin, dest):  # pylint: disable=invalid-name,missing-param-doc,missing-return-doc
        ...
        if key is not None:
            ...
            LOG_S3CACHE.info("Finished downloading.")
            return True
        return False
    ...
    def compressAndUploadDirTarball(self, directory, tarball_path):  # pylint: disable=invalid-name,missing-param-doc
        ...
        LOG_S3CACHE.info("Creating archive...")
    ...
    def uploadFileToS3(self, filename):  # pylint: disable=invalid-name,missing-param-doc,missing-type-doc
        ...
        LOG_S3CACHE.info("Uploading %s to Amazon S3 bucket %s", filename, self.bucket_name)
    ...
    def uploadStrToS3(self, destDir, filename, contents):  # pylint: disable=invalid-name,missing-param-doc
        ...
        LOG_S3CACHE.info("")  # This newline is needed to get the path of the compiled binary, output on a newline.
