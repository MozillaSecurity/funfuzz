    ...
    if is_win_dumping_to_default():
        ...
        while True:
            ...
            if loops > max_loops:
                ...
                FUNFUZZ_LOG.info("make_cdb_cmd waited a long time, but %s never appeared!", str(dump_name))
                return []
    else:
        return []
...
def grab_crash_log(prog_full_path, crashed_pid, log_prefix, want_stack):
    ...
    if dbggr_cmd:
        ...
        if use_logfiles:
            ...
            FUNFUZZ_LOG.info("I don't know what to do with a core file when log_prefix is null")
    ...
    # On Mac, look for a crash log generated by Mac OS X Crash Reporter
        ...
        while True:
            ...
            if loops > max_loops:
                ...
                FUNFUZZ_LOG.info("grab_crash_log waited a long time, but a crash log for %s [%s] never appeared!",
                                 progname, crashed_pid)
                break

    elif platform.system() == "Linux":
        FUNFUZZ_LOG.info("Warning: grab_crash_log() did not find a core file for PID %d.", crashed_pid)
        FUNFUZZ_LOG.info("Note: Your soft limit for core file sizes is currently %d. "
