...
def main():
    ...
    if (sys.argv[1] == '32') and (os.name == 'posix'):
        # 32-bit js shells have only been tested on Linux (i686 or armv71) or Mac (i386) platforms.
        if (os.uname()[0] == 'Linux'):
            if (os.uname()[4] != 'i686') or (os.uname()[4] != 'armv71'):
                raise Exception('32-bit compilation is not supported on non-i686 or non-ARMv71 Linux platforms.')
