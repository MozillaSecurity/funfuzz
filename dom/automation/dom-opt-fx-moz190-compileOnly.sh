...
# FAQ:
# 1: If something screws up, trash the entire existing ~/Desktop/dom-opt-fx-moz190-A folder.
# 2: To clone this script, replace "dom-opt-fx-moz190-A" throughout this script with "dom-opt-fx-moz190-B", "dom-opt-fx-moz190-C" and so on.
# 4: File naming convention is:
#        dom-<opt/debug>-<app>-<branch/trunk>-compileOnly

date
mkdir -p ~/Desktop/dom-opt-fx-moz190-A  #  This will overwrite your existing directory's files.
...
cp ~/fuzzing/dom/automation/mozconfig-opt-fx-moz190 ~/Desktop/dom-opt-fx-moz190-A/fx-moz190-cvs-opt/mozilla/.mozconfig
date
