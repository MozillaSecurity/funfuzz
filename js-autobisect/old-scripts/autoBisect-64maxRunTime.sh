...
case $compileType in
  ...
  "dbg" )
    ...
    # For 10.6:
    sh ../configure --disable-optimize --enable-debug
    make -j2
#    if make -j2; then
...
#    fi
  ...
  "opt" )
    ...
    # For 10.6:
    sh ../configure --enable-optimize --disable-debug
    make -j2
#    if make -j2; then
# Note: this doesn't yet work!
#       # Miscellaneous problems, such as build problems, should be skipped.
...
if ./js-$compileType-tm -j -e maxRunTime=5000 $testcaseFile > tempResult 2>&1; then
  exitCode=$?
