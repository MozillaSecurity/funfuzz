...
case $compileType in
  ...
  "dbg" )
    ...
    # For 10.6:
    CC="gcc-4.2 -arch i386" CXX="g++-4.2 -arch i386" HOST_CC="gcc-4.2" HOST_CXX="g++-4.2" RANLIB=ranlib AR=ar AS=$CC LD=ld STRIP="strip -x -S" CROSS_COMPILE=1 sh ../configure --target=i386-apple-darwin8.0.0 --disable-optimize --enable-debug
    make -j2
#    if make -j2; then
...
#    fi
  ...
  "opt" )
    ...
    # For 10.6:
    CC="gcc-4.2 -arch i386" CXX="g++-4.2 -arch i386" HOST_CC="gcc-4.2" HOST_CXX="g++-4.2" RANLIB=ranlib AR=ar AS=$CC LD=ld STRIP="strip -x -S" CROSS_COMPILE=1 sh ../configure --target=i386-apple-darwin8.0.0 --enable-optimize --disable-debug
    make -j2
#    if make -j2; then
# Note: this doesn't yet work!
#       # Miscellaneous problems, such as build problems, should be skipped.
...
if ([ "$exitCode" = 3 ] || [ "$exitCode" = 1 ] || [ 129 -le "$exitCode" -a "$exitCode" -le 159 ]); then
  if [ "$bad" = bug ]; then
    echo "BAD changeset: hg bisect -b"
    hg bisect -b;
  fi
...
if ([ "$exitCode" = 0 ] || [ 4 -le "$exitCode" -a "$exitCode" -le 6 ]); then
  if [ "$bad" = bug ]; then
    echo "GOOD changeset: hg bisect -g"
    hg bisect -g;
  fi
