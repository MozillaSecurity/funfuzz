...
if ./js-$compileType-$branchType-intelmac -j $testcaseFile > tempResult 2>&1
then exitCode=$?; echo $exitCode; echo SUCCESS;
else exitCode=$?; cat tempResult; echo $exitCode; echo FAILURE
fi
echo -n "exitCode is: "
echo $exitCode
cd ~/tracemonkey/
...
if [ "$bad" = 1 ]; then echo "hg bisect -b"; hg bisect -b; fi
if [ "$bad" = 0 ]; then echo "hg bisect -g"; hg bisect -g; fi
...
# The next two lines are the equivalent of "hg bisect -$good".
if [ "$bad" = 1 ]; then echo "hg bisect -g"; hg bisect -g; fi
if [ "$bad" = 0 ]; then echo "hg bisect -b"; hg bisect -b; fi
rm -rf ~/Desktop/jsfunfuzz-$compileType-$branchType/
exit 0
fi
